...
class SysInfo:
    ...
    @sys.command(pass_context=True)
    @checks.is_owner()
    async def info(self, ctx, *args: str):
        """Summary of cpu, memory, disk and network information
         Usage: info [option]
         Examples:
             sys           Shows all available info
             sys cpu       Shows CPU usage
             sys memory    Shows memory usage
             sys file      Shows full path of open files
             sys disk      Shows disk usage
             sys network   Shows network usage
             sys boot      Shows boot time
         """
        ...
        if not args or args[0].lower() not in options:
            msg = "\n\n".join([cpu_cs, cpu_ps, mem_vs, open_fs, disk_us, net_ios, boot_s])
