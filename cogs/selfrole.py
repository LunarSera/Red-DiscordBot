...
"""selfroles (exclusive-or roles) cog by GrumpiestVulcan
...
JSON = 'data/enigmata/selfrole.json'


class selfroleException(Exception):
    pass

class RolesetAlreadyExists(selfroleException):
    pass

class RolesetNotFound(selfroleException):
    pass

class NoRolesetsFound(selfroleException):
    pass

class RoleNotFound(selfroleException):
    pass

class PermissionsError(selfroleException):
    pass
...
class selfrole:
    ...
    async def selfrole(self, ctx, *, role: str = None):
        if ctx.invoked_subcommand is None:
            if role:
                await ctx.invoke(self.selfrole_add, role=role)
            else:
                await self.bot.send_cmd_help(ctx)

    @selfrole.command(name='list', pass_context=True)
    async def selfrole_list(self, ctx, *, roleset: str = None):
        ...
        except selfroleException as e:
            await self.bot.say(warning(*e.args))

    @selfrole.command(name='set', pass_context=True)
    async def selfrole_add(self, ctx, *, role: str):
        "Assigns a role to you, replacing any other color in the list you selected."
        server = ctx.message.server
        ...
        try:
            ...
            await self.bot.say("You have switched to %s."
                               % (role.name))

        except selfroleException as e:
            await self.bot.say(warning(*e.args))

    @selfrole.command(name='unset', pass_context=True)
    async def selfrole_remove(self, ctx, *, role_or_roleset: str):
        "Removes your role color."
        server = ctx.message.server
        member = ctx.message.author
    ...
    @selfrole.command(name='toggle', pass_context=True)
    async def selfrole_toggle(self, ctx, *, role_or_roleset: str):
    ...
    async def selfroleset(self, ctx):
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='addroleset', pass_context=True)
    async def selfroleset_addroleset(self, ctx, *, name: str):
        "Adds a roleset."
        server = ctx.message.server
        try:
            if len(name.split()) > 1:
    ...
    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='rmroleset', pass_context=True)
    async def selfroleset_rmroleset(self, ctx, *, name: str):
        "Removes a roleset."
        server = ctx.message.server
        try:
    ...
    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='renroleset', pass_context=True)
    async def selfroleset_renroleset(self, ctx, oldname: str, newname: str):
        "Renames a roleset."
        server = ctx.message.server
        try:
    ...
    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='audit', pass_context=True)
    async def selfroleset_audit(self, ctx):
        "Shows members with more than one role in a selfrole roleset"
        lines = []
        server = ctx.message.server
        try:
    ...
    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='addroles', aliases=['addrole'], pass_context=True)
    async def selfroleset_addroles(self, ctx, roleset: str, *, roles: str):
        """Adds one or more roles to a selfrole roleset.
    ...
    @checks.mod_or_permissions(administrator=True)
    @selfroleset.command(name='rmroles', aliases=['rmrole'], pass_context=True)
    async def selfroleset_rmroles(self, ctx, roleset: str, *, roles: str):
        """Removes one or more roles from a selfrole roleset.
